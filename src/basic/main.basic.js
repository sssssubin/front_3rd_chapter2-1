...
function calculateCartTotal(cartItems) {
  ...
  return cartItems.reduce((total, item) => {
    ...
    const discount =
      quantity >= CONFIG.CART_DISCOUNT_THRESHOLD
        ? CONFIG.DISCOUNT_RATES[product.id] || 0
        : 0;
...
function calculateCartState(cartItems, currentDate) {
  ...
  if (totalItemCount >= CONFIG.CART_DISCOUNT_THRESHOLD) {
    discountedTotal *= 1 - CONFIG.CART_DISCOUNT_RATE;
  }

  // 대량 구매 할인 적용
  discountedTotal = applyBulkPurchaseDiscount(discountedTotal, totalItemCount);

  // 화요일 할인 적용
  const finalTotal = applyTuesdayDiscount(discountedTotal, currentDate);
...
function renderDiscountRate() {
  ...
  if (!discountDisplay) {
    discountDisplay = createElement('span', {
      id: 'discount-display',
  ...
  if (state.discountRate > 0) {
    discountDisplay.textContent = `(${(state.discountRate * 100).toFixed(
      1,
    )}% 할인 적용)`;
    const loyaltyPointsDisplay = document.getElementById('loyalty-points');
    if (loyaltyPointsDisplay) {
      domElements.cartTotalDisplay.insertBefore(
        discountDisplay,
        loyaltyPointsDisplay,
      );
    } else {
      domElements.cartTotalDisplay.appendChild(discountDisplay);
    }
  } else {
    if (discountDisplay.parentNode) {
      discountDisplay.parentNode.removeChild(discountDisplay);
    }
  }
}
